...
const CheckoutsPage = () => {
  ...
  const placeOrder = async () => {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i]?.colorId == null || data[i]?.colorId == "") {
        formData.append(`color_id[${i}]`, 1);
      } else {
        formData.append(`color_id[${i}]`, data[i].colorId);
      }
      formData.append(`size_id[${i}]`, data[i].sizeid);
      // formData.append(`size_id[${i}]`, 1);
      // formData.append(`size_id[${i}]`, cart[i].size);
      formData.append(`main_price[${i}]`, data[i].value);
      formData.append(`price[${i}]`, data[i].value);
      formData.append(`quantity[${i}]`, data[i].qty);
    ...
    if (payment_type == 1 && order_data.success == 1) {
  ...
  async function displayRazorpay(order_id, total_amount) {
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        try {
          let myRes = await axios.post(get_payment_id, formData, {
            headers: {
              Accept: "application/x.uniform.v1+json",
            },
          });
